...
def PostDetailView(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            donation.save()
            send_mail('Corona Rangers has some great news for you', f' {donation.donor} ({request.user.email}) wants to donate  {qty} { donation.category}', settings.EMAIL_HOST_USER, [
                      f'{post.author.email}'], fail_silently=False)
            messages.success(
                request, f'We have notified the NGO, thankyou for the donation.The NGO will contact you')
            return redirect('dashboard')
        else:
            pass
    else:
